  ...
  add: async (req, res) => {
    try {
      ...
      if (errArr.length > 0) {
        ...
        res.status(200).send(SendResponse(false, 'Data Added Successfully', result));
    }
  }catch (e) {
  ...
  get: async (req, res) => {
    ...
    getById: async (req, res) => {
      try {
        ...
        let result = await CourseMdel.findbyId(id);
        res.status(200).send(SendResponse(true, '', result));
      } catch (e) {
      ...
      del: (req, res) => {
        try {
          ...
          CourseModel.findByIdAndDelete(id)
            .then(() => {
              res.status(200).send(SendResponse(true,'Data Delete Successfully',))
            })
